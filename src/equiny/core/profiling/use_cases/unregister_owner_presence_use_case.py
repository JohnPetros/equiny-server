...
from equiny.core.profiling.domain.events import OwnerPresenceUnregisteredEvent
...
class UnregisterOwnerPresenceUseCase:
    ...
    def execute(self, owner_id: str) -> None:
        ...
        owner.exit_presence()
        self._owners_repository.replace(owner)
        owner_matches = self._find_owner_matches(owner.id)
        self._broker.publish(
            OwnerPresenceUnregisteredEvent(owner.id.value, owner_matches)
        )
